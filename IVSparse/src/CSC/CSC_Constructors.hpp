...
namespace IVSparse {
    ...
    SparseMatrix<T, indexT, 1, columnMajor>::SparseMatrix(Eigen::SparseMatrix<T> &mat) {
        ...
        } catch (std::bad_alloc &e) { std::cerr << "Allocation failed: " << e.what() << '\n'; }
    ...
    template <typename T, typename indexT, bool columnMajor>
    SparseMatrix<T, indexT, 1, columnMajor>::SparseMatrix(const IVSparse::SparseMatrix<T, indexT, 1, columnMajor> &other) { *this = other; }
    ...
    SparseMatrix<T, indexT, 1, columnMajor>::SparseMatrix(IVSparse::SparseMatrix<T, indexT, compressionLevel2, columnMajor> &other) {
        ...
        if constexpr (compressionLevel2 == 2) { temp = other.toCSC(); }
        else if constexpr (compressionLevel2 == 3) { temp = other.toCSC(); }
    ...
    SparseMatrix<T, indexT, 1, columnMajor>::SparseMatrix(typename IVSparse::SparseMatrix<T, indexT, 1, columnMajor>::Vector &vec) {
        ...
        if (vec.byteSize() == 0) {
            ...
            // allocate outerPtr
            try { outerPtr = (indexT *)malloc((outerDim + 1) * sizeof(indexT)); }
            catch (std::bad_alloc &e) { std::cerr << "Allocation failed: " << e.what() << '\n'; }
    ...
    template <typename T, typename indexT, bool columnMajor>
    SparseMatrix<T, indexT, 1, columnMajor>::SparseMatrix(std::vector<typename IVSparse::SparseMatrix<T, indexT, 1, columnMajor>::Vector> &vecs) {
    ...
    SparseMatrix<T, indexT, 1, columnMajor>::SparseMatrix(std::vector<std::tuple<indexT2, indexT2, T2>> &entries, uint32_t num_rows, uint32_t num_cols, uint32_t nnz) {
        ...
        for (size_t i = 0; i < entries.size(); i++) {
            ...
            if (std::get<1>(entries[i]) != OuterIndex) {
                if (OuterIndex != -1) { outerPtr[OuterIndex + 1] = count - 1; }
