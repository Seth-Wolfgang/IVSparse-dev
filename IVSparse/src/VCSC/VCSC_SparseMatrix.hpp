...
namespace IVSparse
{
    ...
    /**
     * The VCSC Sparse Matrix Class is a version of the CSC format. The difference is
     * that CSC is base CSC but VCSC is CSC with run length encoding and a vector that
     * keeps track of the number of occurances of a value in a column. This allows for
     * fast level 1 BLAS operations and still compresses more with redundant data than CSC.
     */
    ...
    public:
        ...
        /**
         * Raw CSC Constructor \n \n
         * This constructor takes in raw CSC storage format pointers and converts it to a VCSC matrix.
         * One could also take this information and convert to an Eigen Sparse Matrix and then to a VCSC matrix.
         */
        template <typename T2, typename indexT2>
         ...
         * @returns The number of indices (nonzeros) in a given vector in a VCSC Matrix
         */
         ...
         */
        typename IVSparse::SparseMatrix<T, indexT, 2, columnMajor>::Vector getVector(uint32_t vec);
