...
namespace IVSparse {
    ...
    void SparseMatrix<T, indexT, compressionLevel, columnMajor>::calculateCompSize() {
        ...
        compSize = 16; // for pointer arrays of data and endPointers
        ...
        for (uint32_t i = 0; i < outerDim; i++) {
            // std::cout << "i : " << i << " " << *((uint8_t**)endPointers + i) - *((uint8_t**)data + i) << std::endl;
            compSize += *((uint8_t**)endPointers + i) - *((uint8_t**)data + i);
        }
        // std::cout << "IVCSC COMP SIZE: " << compSize << std::endl;
    }
