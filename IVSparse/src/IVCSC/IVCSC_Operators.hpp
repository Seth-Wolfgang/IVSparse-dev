...
namespace IVSparse {
    ...
    T SparseMatrix<T, indexT, compressionLevel, columnMajor>::operator()(uint32_t row, uint32_t col) {
        ...
        for (typename SparseMatrix<T, indexT, compressionLevel, columnMajor>::InnerIterator it(*this, vector); it; ++it) {
            if (it.getIndex() == (indexT)index) { return it.value(); }
        }
    ...
    std::ostream &operator<<(std::ostream &os, IVSparse::SparseMatrix<T, indexT, compressionLevel, columnMajor> &mat) {
        ...
        for (size_t i = 0; i < mat.cols(); i++) {
            for (typename IVSparse::SparseMatrix<T, indexT, compressionLevel, columnMajor>::InnerIterator it(mat, i); it; ++it) {
