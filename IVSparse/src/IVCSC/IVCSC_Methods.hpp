...
namespace IVSparse {
    ...
    void SparseMatrix<T, indexT, compressionLevel, columnMajor>::write(const char *filename) {
        ...
        // write each vector
        for (uint32_t i = 0; i < outerDim; i++) { fwrite(data[i], 1, (char *)endPointers[i] - (char *)data[i], fp); }
    ...
    IVSparse::SparseMatrix<T, indexT, 2, columnMajor> SparseMatrix<T, indexT, compressionLevel, columnMajor>::toVCSC() {
        ...
        IVSparse::SparseMatrix<T, indexT, 2, columnMajor> mat(values, indices, colPtrs, numRows, numCols, nnz);
    ...
    template <typename T, typename indexT, uint8_t compressionLevel, bool columnMajor>
    void SparseMatrix<T, indexT, compressionLevel, columnMajor>::append(typename IVSparse::SparseMatrix<T, indexT, compressionLevel, columnMajor>::Vector &vec) {
        // check if the matrix is empty
        if (compSize == 0) [[unlikely]] { *this = IVSparse::SparseMatrix<T, indexT, compressionLevel, columnMajor>(vec); }
        else {
    ...
    std::vector<typename IVSparse::SparseMatrix<T, indexT, compressionLevel, columnMajor>::Vector> SparseMatrix<T, indexT, compressionLevel, columnMajor>::slice(uint32_t start, uint32_t end) {
        ...
        std::vector<typename IVSparse::SparseMatrix<T, indexT, compressionLevel, columnMajor>::Vector> vecs(end - start);

        // grab the vectors and add them to vecs
        for (uint32_t i = start; i < end; ++i) {
            // make a temp vector
            IVSparse::SparseMatrix<T, indexT, compressionLevel, columnMajor>::Vector temp(*this, i);
