...
namespace CSF {
    ...
    template <typename T, typename indexT, uint8_t compressionLevel, bool columnMajor>
    uint32_t SparseMatrix<T, indexT, compressionLevel, columnMajor>::Vector::getValueArraySize() { return valueArraySize; }

    template <typename T, typename indexT, uint8_t compressionLevel, bool columnMajor>
    T* SparseMatrix<T, indexT, compressionLevel, columnMajor>::Vector::getValueArray() { return valueArray; }
    ...
    void SparseMatrix<T, indexT, compressionLevel, columnMajor>::Vector::activatePerformanceVecs() {
        ...
        try {
            ...
            counts = (uint32_t*)malloc(sizeof(uint32_t) * valueArraySize);
            memcpy(counts, countsVector.data(), sizeof(uint32_t) * valueArraySize);
        }
        catch (std::bad_alloc& e) {
            std::cout << "Error: " << e.what() << std::endl;
            exit(1);
    ...
    typename SparseMatrix<T, indexT, compressionLevel, columnMajor>::Vector SparseMatrix<T, indexT, compressionLevel, columnMajor>::Vector::operator=(typename SparseMatrix<T, indexT, compressionLevel, columnMajor>::Vector& other) {
        ...
        if (other.performanceVectors) {
            ...
            counts = (uint32_t*)malloc(sizeof(uint32_t) * other.valueArraySize);

            memcpy(counts, other.counts, sizeof(uint32_t) * other.valueArraySize);
