/**
 * @file CSF1_Vector_Methods.hpp
 * @author Skyler Ruiter and Seth Wolfgang
 * @brief CSF1 Vector Methods
 * @version 0.1
 * @date 2023-07-03
 */
...
namespace CSF {
    ...
    bool SparseMatrix<T, indexT, 1, columnMajor>::Vector::operator==(typename SparseMatrix<T, indexT, 1, columnMajor>::Vector &vec) {
        ...
        if (length != vec.length || nnz != vec.nnz) { return false; }

        // check if the values and indices are the same
        for (uint32_t i = 0; i < nnz; i++) { if (vals[i] != vec.vals[i] || innerIdx[i] != vec.innerIdx[i]) { return false; } }
