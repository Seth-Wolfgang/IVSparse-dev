...
int main() {
    ...
    //Create an Eigen matrix so we can convert to CSF
    ...
    CSF::SparseMatrix<double> csfSparseMatrix(eigenMatrix);             //Create a CSF::SparseMatrix from the Eigen::MatrixXd

    /**
     * The CSF::SparseMatrix class has an iterator that can be used to traverse through the data.
     * The iterator is a nested class within the CSF::SparseMatrix class, so it is important to specify 'typename' before the iterator.
    */


    /**
     * The iterator works by moving through the data in the CSF::SparseMatrix.
    ...
    for (typename CSF::SparseMatrix<double>::InnerIterator it(csfSparseMatrix, 0); it; ++it) {
        std::cout << it.value() << " ";
    }
    ...
    */
    ...
    for (uint32_t i = 0; i < csfSparseMatrix.outerSize(); ++i) {
        for (typename CSF::SparseMatrix<double>::InnerIterator it(csfSparseMatrix, i); it; ++it) {
            std::cout << it.value() << " ";
        }
        std::cout << std::endl;
    }
     ...
     * how data is stored in a CSF matrix. **If you change any value then that value will be changed for ALL occurrences of the value changed.**
     * 
     * Ex. 
    ...
    */
    ...
    CSF::SparseMatrix<double, int, 3> csfMatrix(eigenMatrix);

    std::cout << csfSparseMatrix << std::endl;

    //Now we will change the first value in the matrix to a 17
    typename CSF::SparseMatrix<double>::InnerIterator myIter(csfSparseMatrix, 0);
    myIter.coeff(17);
    ...
    */
    ...
    /**
     * 
     * The InnerIterator is also capable of iterating across CSF::Vector 
     * 
     */

    typename CSF::SparseMatrix<double>::Vector myVec = csfSparseMatrix.getVector(0);
    /**
     * To iterate over the vector, you can simply
    */

   for(typename CSF::SparseMatrix<double>::InnerIterator it(myVec); it; ++it) {
       std::cout << it.value() << " ";
   }
